...
HDFS_OUTPUT="/customer_etl/output/loyalty_snapshot"
FINAL_CSV="/opt/spark-apps/shared_output/customer_etl/loyalty_snapshot_${RUN_DATE}.csv"
#FINAL_CSV="/opt/spark-apps/shared_output/customer_etl/loyalty_snapshot_${RUN_DATE}"
...
if grep -qE 'docker|containerd' /proc/1/cgroup; then
  ...
  spark-submit --master spark://spark-master:7077 /opt/spark-apps/customer_etl/scripts/customer_ETL.py "$RUN_DATE"
  ...
  docker exec spark-master spark-submit /opt/spark-apps/customer_etl/scripts/customer_ETL.py "$RUN_DATE"
