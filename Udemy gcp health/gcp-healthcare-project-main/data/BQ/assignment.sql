...
CREATE TABLE IF NOT EXISTS `etl-project-003.gold_dataset.financial_metrics` AS
SELECT 
...
CREATE TABLE IF NOT EXISTS `etl-project-003.gold_dataset.payor_performance` AS
SELECT 
    c.PayorID,
    c.PayorType,
    COUNT(DISTINCT c.Claim_Key) AS TotalClaims,
...
FROM `etl-project-003.silver_dataset.claims` c
